...
registerLicense(import.meta.env.VITE_APP_CALENDAR_KEY);
...
const EventCalendar = () => {
  ...
  const eventSettings = {
    dataSource: extend([], filteredData.planned, null, true),
  };
  const resourceData = [
  ...
  function generateCalendarData() {
    ...
    for (const data of collections) {
      ...
      data.StartTime = new Date(
        newTime - diff + (data.IsAllDay ? 0 : hour * 60 * 60 * 1000)
      );
      data.EndTime = new Date(
        data.StartTime.getTime() +
          (data.IsAllDay ? 24 * 60 * 60 * 1000 : 60 * 60 * 1000)
      );
      ...
      saveButtonRef.current.onclick = () => {
        if (calendarNameObj) {
          ...
          if (newValue.length > 0) {
            ...
            calendarsListObj.current.dataSource = extend(
              [],
              calendars,
              null,
              true
  ...
  const removeCalendar = (args) => {
    ...
    appointmentData = appointmentData.filter(
      (item) => item.CalendarId !== args.data.id
    );
    selectedCalendars = getSelectedCalendars();
    filteredData = getFilteredData();
    scheduleObj.current.eventSettings.dataSource = extend(
      [],
    ...
    if (isAdd) {
      if (calendarNameObj) {
        ...
        const newItem = {
          name: newValue,
          id: newId,
          color: colorPickerObj.current.value,
          isSelected: true,
        };
  ...
  const applyBackgroundColors = () => {
    calendars.forEach((calendar) => {
      const listItem = calendarsListObj.current.element.querySelector(
        `[data-uid="${calendar.id}"]`
      );
      if (listItem) {
        const checkboxFrame =
          listItem.querySelector(`.e-checkbox-wrapper .e-frame.e-check,
  ...
  const onToolbarItemClicked = (args) => {
    ...
    switch (args.item.cssClass) {
      ...
      case "e-create":
        ...
        break;
      case "e-previous":
        scheduleObj.current.changeDate(
          scheduleObj.current.activeView.getNextPreviousDate("Previous")
        );
        break;
      case "e-next":
        scheduleObj.current.changeDate(
          scheduleObj.current.activeView.getNextPreviousDate("Next")
        );
        break;
      case "e-today":
  ...
  const onScheduleActionComplete = (args) => {
    if (
      ...
      if (
        args.requestType === "dateNavigate" &&
        resetTime(calendarObj.current?.value) !==
          resetTime(scheduleObj.current.selectedDate)
      ) {
      ...
      for (const event of events) {
        let calendar = selectedCalendars.items.find(
        ...
        if (isNullOrUndefined(calendar)) {
          ...
          gridObj.current.dataSource = extend(
            [],
            filteredData.unPlanned,
            null,
            true
          );
        }
  ...
  const updateDateRange = () => {
    ...
    if (scheduleObj.current) {
      ...
      if (dateRange !== "" && toolbarObj) {
        const dateRangeElement = toolbarObj.current.element.querySelector(
          ".e-date-range .e-tbar-btn-text"
        );
        toolbarObj.current.element
          .querySelector(".e-date-range .e-tbar-btn")
          .setAttribute("aria-label", dateRange);
      ...
      <div className="calendar-list-item">
        ...
        {data.id !== 1 && (
          <div className="calendar-buttons">
            <span
              id="calendar-edit-btn"
              className="e-icons e-edit"
              data-calendar-id={data.id}
            ></span>
            <span
    ...
    if (args.type === "Editor") {
      if (!args.element.querySelector(".custom-field-row")) {
        ...
        formElement.firstChild.insertBefore(
          row,
          args.element.querySelector(".e-resources-row")
        );
        ...
        const calendarDDL = args.element.querySelector(".calendar-ddl input")
          .ej2_instances[0];
  ...
  const eventRendered = (args) => {
    const categoryColor =
      selectedCalendars.items[
        selectedCalendars.ids.indexOf(args.data.CalendarId)
      ].color;
      ...
      <div className="dialogContent">
        ...
        <div className="dialog-content">
          <TextBoxComponent
            ref={calendarNameObj}
            id="text-box"
            placeholder="Enter the calender name"
          />
  ...
  const dialogFooterTemplate = () => {
    return (
      <button
        ...
        data-ripple="true"
        onClick={updateTextValue}
      ></button>
    );
  };
  const unplannedSidebarClosed = () => {
    const unplannedElement =
      unPlannedSidebarObj.current.element.parentElement.querySelector(
        ".unplanned-container"
      );
    if (unplannedElement) {
  ...
  return (
    <div
      id="event-calendar-sample"
      className="control-section event-calendar-control-section"
    >
        ...
        <div>
          ...
          >
            <ItemsDirective>
              ...
              />
              <ItemDirective
                prefixIcon="e-chevron-left"
                tooltipText="Previous Week"
                cssClass="e-previous"
              />
              <ItemDirective
                prefixIcon="e-chevron-right"
        ...
        <div className="main-content" id="main-text">
          <div className="sidebar-content">
            ...
            <div className="unplanned-container">
              <div
                id="plannedOpen"
                className="e-icons e-chevron-left-double"
              ></div>
              <SidebarComponent
              ...
              >
                <div id="unplanned-events-toolbar">
                  <button
                    className="e-icons e-exit-full-screen"
                    title="Open/Close Sidebar"
                    onClick={unplannedSideBarCollapse}
                  ></button>
                ...
                <div className="unplanned-text-containers">
                  <GridComponent
                    ref={gridObj}
                    dataSource={extend([], filteredData.unplanned, null, true)}
                  >
                    <ColumnsDirective>
                      <ColumnDirective
                        field="Subject"
